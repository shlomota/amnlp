...
def evaluate_verbose(model, dataset, iter_lim=10, shuffle=True, self_attention=False):
    ...
    for i, (M_s, v_q, y_true) in enumerate(g):
        ...
        if self_attention:
            y_logits, A = model(M_s)
            y_logits = torch.stack([y_logits[i, v_q[i], :] for i in range(len(v_q))])
            A = torch.stack([A[i, v_q[i], :] for i in range(len(v_q))])
        else:
            y_logits, A = model(M_s, v_q)
        _, y_pred = torch.max(y_logits.data, 1)
...
def higlight_samples(model, dataset, sample_size=20, correct_only=False, self_attention=False):
    eval_df, attention_df = evaluate_verbose(model, dataset, iter_lim=100, self_attention=self_attention)
    if correct_only:
